...
//    }
    ...
    self.mobileTextField.inputAccessoryView = self.keyboardToolbar;
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if ([[defaults objectForKey:USER_ADDRESS_EXISTS] isEqualToString:@"True"]) {
        ...
        self.mobileTextField.text = [defaults objectForKey:USER_MOBILE];
...
- (void)doneButtonTapped:(UITapGestureRecognizer *)sender {
    ...
    if (![self.mobileTextField.text isEqualToString:@""]){
        [defaults setObject:self.mobileTextField.text forKey:USER_MOBILE];
    }
    
    
    [self.delegate didSubmitAddressInfo:defaults];
...
- (void)skipButtonTapped:(UITapGestureRecognizer *)sender {
    ...
    //[self.delegate didDismissViewController:nil];
    [self dismissViewControllerAnimated:true completion:nil];
}
...
- (void)textFieldDidChange:(UITextField *)textField {
    ...
    if (self.streetTextField.text.length > 0
        ...
        && self.zipcodeTextField.text.length == 4) {
