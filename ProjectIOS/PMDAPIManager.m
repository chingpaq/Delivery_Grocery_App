                   ...
                   failureBlock:(void (^)(NSError *))failureBlock
{
    ...
    if (customerDict!=nil){
        NSData *postData = [NSJSONSerialization dataWithJSONObject:customerDict options:0 error:&error];
...
- (void)deletePayMayaCustomer:(NSString*)customerID
    ...
    NSError *error = nil;
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@/deleteCustomer", self.baseUrl]];
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:url];
    [request setValue:@"application/json" forHTTPHeaderField:@"Content-Type"];
    [request setHTTPMethod:@"POST"];
    
    NSDictionary*customerDict= @{@"id":customerID,
                                 };
    NSData *postData = [NSJSONSerialization dataWithJSONObject:customerDict options:0 error:&error];
    [request setHTTPBody:postData];
...
- (void)deletePayMayaCustomerCard:(NSString*)cardID
    ...
    NSError *error = nil;
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@/deleteCard", self.baseUrl]];
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:url];
    [request setValue:@"application/json" forHTTPHeaderField:@"Content-Type"];
    [request setHTTPMethod:@"POST"];
    
    NSDictionary*customerDict= @{@"customerId":customerID,
                                 @"cardId": cardID
                                 };
